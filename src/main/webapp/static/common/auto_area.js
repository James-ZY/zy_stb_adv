var auto_area = {
    ...
    appRunList: function (runList, v) {//自动搜索应用
        ...
        for (i in searchValue) {
            ...
            runSearchSelfCode = searchValue[i].selfcode;
            ...
            if (runSearchName.indexOf(v) >= 0 || runSearchPinyin.indexOf(v) >= 0 || runSearchPinyin.toLowerCase().indexOf(v) >= 0 ) {  
            loaded[i] = 1;
                temp += '<a class="area_menu" href="javascript:;" data-flag=1 data-code="' + runSearchCode + '" data-selfcode="'+runSearchSelfCode+'" data-name="' + runSearchName + '" onclick="selectProvince(\'sub\',this,\'\')"><em>' + runSearchPinyin.replace(v, "<b>" + v + "</b>") + '</em>' + runSearchName.replace(v, "<b>" + v + "</b>") + '</a>';
                if (++n > 10) break;
            }
        }
        if (temp) {// 搜索到应用则显示
