...
$(function(){
    ...
    function ad_gjPlay(pathData,position,range,node){
        ...
        $('.ad_gj dl').css({"left":psLeft,"top":psTop,"position":"absolute","margin":"auto"});
    ...
    function ad_gdPlay(pathData,showtime,showCount,roolposition,position,range,Velocity,gdFx,node){
		...
		/*var showTime=showtime;
		 var moveStyle;
		 var Velocity=parseFloat(Velocity/50);
		 var showHtml='';
		 var contentWidth;
		 var showStyle="ad_gd_style1";
		 $.each(pathData,function(commentIndex,comment){
		 showHtml+='<li id='+commentIndex+'><img src='+comment+'></li>';
		 });
		 if(position.startY==position.endY){
		 //水平方向
		 if(position.startX<position.endX){
		 contentWidth=position.endX-position.startX;
		 moveStyle="right";
		 showStyle="ad_gd_style1";
		 $('#ad_gd').css({"left":position.startX,"top":position.startY,"width":contentWidth});
		 }else{
		 contentWidth=position.startX-position.endX;
		 moveStyle="left";
		 showStyle="ad_gd_style1";
		 $('#ad_gd').css({"left":position.endX,"top":position.startY,"width":contentWidth});
		 }
		 $('#ad_gd').addClass(showStyle);
		 $('.ad_gd_content').html(showHtml);
		 }
		 if(position.startX==position.endX){
		 //垂直方向
		 if(position.startY<position.endY){
		 contentWidth=position.endY-position.startY;
		 moveStyle="down";
		 showStyle="ad_gd_style3";
		 $('#ad_gd').css({"left":position.startX,"top":position.startY,"height":contentWidth});
		 }else{
		 contentWidth=position.startY-position.endY;
		 moveStyle="up";
		 showStyle="ad_gd_style3";
		 $('#ad_gd').css({"left":position.startX,"top":position.endY,"height":contentWidth});
		 }
		 $('#ad_gd').addClass(showStyle);
		 $('.ad_gd_content').html(showHtml);
		 setHeight();
