$(function() {
    ...
    function judage_advOne(){
        ...
        var b=document.getElementById("adcombo_id");
        ...
        var comboId=b.options[b.selectedIndex].value;
        var startDate=$("#startDate").val();
        var endDate=$("#endDate").val();
        var id=$("#id").val();
        var data={"id":id,"startDate":startDate,"endDate":endDate,"comboId":comboId,"advId":advId,"childAdType":childAdType};
        var postData=JSON.stringify(data);
    ...
    function  getComboInfo(e) {
        var comboFlag = $("#comboFlag").val();
        var typeId = $("#adTypeId").val();
        if(comboFlag ==1){
            $("#timeShowSet").css("display","block");
            $.ajax({
                url:host+"/adv/combo/getComboInfo",
                async: false,
                type:"POST",
                data:e,
                contentType: "application/json; charset=utf-8",
                ...
                success:function(data){
                    if(null != data) {
                        ...
                        if(typeId == 4){
                            $("#showTimeSet").css("display","block");
                            $("#rollSet").css("display","none");
                            $("#showTime").val(data.showTime);
                            $("#intervalTime").val(data.intervalTime);
                            $("#showCount").val(data.showCount);
                            $("#pictureTimes").val("");
                            $("#pictureInterval").val("");
                        }else if(typeId ==5){
                            $("#showTimeSet").css("display","block");
                            $("#rollSet").css("display","block");
                            $("#showTime").val(data.showTime);
                            $("#intervalTime").val(data.intervalTime);
                            $("#showCount").val(data.showCount);
                            $("#pictureTimes").val(data.pictureTimes);
                            $("#pictureInterval").val(data.pictureInterval);
                        }else{
                            $("#showTimeSet").css("display","none");
                            $("#rollSet").css("display","none");
                            $("#showTime").val("");
                            $("#intervalTime").val("");
                            $("#showCount").val("");
    ...
    //获取子广告类型
    function getchlidType(e){
        $.ajax({
            url:host+"/adv/adelement/getAdType",
            async: false,
            type:"POST",
            data:e,
            contentType: "application/json; charset=utf-8",
            ...
            success:function(data){
                ...
                $("#adTypeId").val(data.typeId);
                $("#comboFlag").val(data.comboFlag);
                if(data.typeId == 5){
                ...
                if(data !=null && data.typeId == 8 && data.childList!=undefined && data.childList.length!=0){
                    var chlidType = data.childTypeId;
                    var len=data.childList.length;
                    var html='<option>'+accipiter.getLang_(messageLang,"userform.select")+'</option>';
                    for(var i=0;i<len;i++){
    ...
    function getSelectData(id,ClassId,comboId,resolution){
            ...
            $.ajax({
                ...
                success:function(data){
                    ...
                    if(resolution=="0"){
                        ...
                        $(".pcshowTime").change(function(){
                            var min = $("#psTs").val();
                            var selLen = $(this).parent().parent().parent().parent().find("li[class='item selected']").length;
                            var iptVal = parseInt($(this).val());
                            if(selLen == 1 && iptVal > 0 && iptVal<parseInt(min)){
                                    $(this).val(min);
                            }
                            if(selLen > 1){
                                if(iptVal<parseInt(min)){
                                    $(this).val(min);
                                }
                            }
        ...
        if((id!=""&&ClassId==adClassId) ||(id==""&&comboId!="")){
    ...
    function judage(){
        ...
        if(status==3){
            $(".control-group").find("input").attr("disabled",true);
            $(".control-group").find("select").attr("disabled",true);
            $(".advStandard").attr("disabled",false);
            $(".advHd").attr("disabled",false);
            $("#startDate").attr("disabled",false);
            $("#endDate").attr("disabled",false);
            $("#adv_isFlag").attr("disabled",false);
            $("#playTime").attr("disabled",false);
            $("#adCategoryId").attr("disabled",false);
            $("#adCategoryName").attr("disabled",false);
            var adTypeId = $("#adTypeId").val();
            if(adTypeId ==5){
                $("#sdFx").attr("disabled",false);
                $("#hdFx").attr("disabled",false);
            }
        }
        $("#sdversion").attr("disabled",false);
        $("#hdversion").attr("disabled",false);
    }
    judage();
    /******************高标清资源切换和坐标展示**********/
    function showPointAndResources(node,show){
        if($("#adcombo_id").find("option:selected").val()!=""){
            var resolution="0";
            var comboId = $("#comboId").val();
            /***控制索材显示***/
            if(node=="advHd"){
    ...
    function alertShowInfo(){
        ...
        var comboId=$("#comboId").val();
        var adTypeId = $("#adTypeId").val();
        if(adTypeId ==2 || adTypeId ==3 ||adTypeId ==4 || adTypeId ==5 || adTypeId ==10){
            $("#timeShowSet").css("display", "block");
        }
        if(adTypeId == 4){
            $("#showTimeSet").css("display","block");
        ...
        if((adId!="") ||(adId==""&&comboId!=""&&comboId!=undefined)){
            getDataByAmend();
            var Adv="";
            var a=document.getElementById("adcombo_id");
            Adv=a.options[a.selectedIndex].value;
    ...
    $('#adcombo_id').change(function() {
        $("#comboId").val($(this).val());
        $("#path").val("");
        $("#sdShowParam").val("");
        $("#hdpath").val("");
        $("#hdShowParam").val("");
