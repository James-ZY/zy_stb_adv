...
$(function(){
	...
	if($('#id').attr("value")!=""){
		...
		var data={"id":$(".select").children("option:selected").val(),"flag":$("#flag").children("option:selected").val(),"rollFlag":$("#rollFlag").children("option:selected").val()};
    ...
    $("#flag").change(function(){
		...
		var info=$(".select").children("option:selected").text();
		if(control!=""&&typeAdv!=""){
			var data={"id":typeAdv,"flag":control,"rollFlag":$("#rollFlag").children("option:selected").val()};
	...
	$(function(){
		if($("#flag").children("option:selected").val()!=""){
			var control= parseInt($("#adType").children("option:selected").val().getAdvType());
			var info=$("#adType").children("option:selected").text();
			var data={"id":$("#adType").children("option:selected").val(),"flag":$("#flag").children("option:selected").val(),"rollFlag":$("#rollFlag").children("option:selected").val()};
			fileLimit(control,info,getSize(data));
		}
	});
	...
	$(".select").change(function(){
		if($("#flag").children("option:selected").val()!=""){
			...
			$("#adType").children('option').attr("selected",false);
			$("#adType").children('option:eq(0)').attr("selected",true);
			$("#adType").select2();
		}
	});
    /*-------GIF与feiGIF切换*/
	$(".selectPictureType").change(function(){
		...
		if(control==0){
			...
			var info=$(".select").children("option:selected").text();
			if(control!=""&&flag!=""){
				var data={"id":$(".select").children("option:selected").val(),"flag":flag,"rollFlag":$("#rollFlag").children("option:selected").val()};
				fileLimit(control,info,getSize(data));	
			}
			return;
		}else{
     ...
     * 默认值
     * */
    var heightSize=50;    //图片高度范围（30-100），默认值50,
    var heightRange={min:30,max:100};
    var fontSize=40;
    var fontStyle="宋体";
    var fontBold=1;//是否加粗
    var fontItalic=0;//是否倾斜
    ...
    function setDefault(e){
        if(e.hasClass("set-font-style")){
            var fontStleText=$('.set-font-style li[for="'+fontStyle+'"]').text();
        ...
        if(e.hasClass("set-font-size")){
            ...
            var fb="";
            if(fontBold==0){
                fb=accipiter.getLang("no");
            }else{
                fb=accipiter.getLang("yes");
            }
            $(".set-chose").find(".font-bold").attr("value",fb);

            //设置是否倾斜
            var fi="";
            if(fontItalic==0){
                fi=accipiter.getLang("no");
            }else{
                fi=accipiter.getLang("yes");
            }
            $(".set-chose").find(".font-italic").attr("value",fi);
     ...
     * */
     ...
     function createFontLanguage(e){
         html='<li><label>'+accipiter.getLang_(messageLang,"set.fontStyel")+':</label><div class="set-chose-content set-font-style"><input type="text" class="chose-btn chose-fontStyle-btn up" readonly><i class="icon-down"></i><ul>';
         $.ajax({
             url :host + "/sys/param/getFontParams",
             type :"post",
             async:false,
             data :JSON.stringify({}),
             contentType : "application/json; charset=utf-8",
             ...
             success:function(data){
                 $.each(data,function(commentIndex,comment){
                     html += '<li for="'+comment+'" class="chose-item">'+comment+'</li>';
                 });
     ...
     function createFontStyle(e){
         ...
         html+='<li><label>'+accipiter.getLang_(messageLang,"set.isBold")+':</label>' +
             '<div class="set-chose-content set-font-bold">' +
             '<input type="text" class="chose-btn font-bold up" readonly><i class="icon-down"></i>'+
             '<ul>' +
             '<li class="chose-item" value="0">'+accipiter.getLang("no")+'</li>' +
             '<li class="chose-item" value="1">'+accipiter.getLang("yes")+'</option>' +
             '</ul>' +
             '</div>';

         html+='<li><label>'+accipiter.getLang_(messageLang,"set.isItalic")+':</label>' +
             '<div class="set-chose-content set-font-italic">' +
             '<input type="text" class="chose-btn font-italic up" readonly><i class="icon-down"></i>'+
             '<ul>' +
             '<li class="chose-item" value="0">'+accipiter.getLang("no")+'</li>' +
             '<li class="chose-item" value="1">'+accipiter.getLang("yes")+'</option>' +
             '</ul>' +
      ...
      * */
       ...
       * */
       function createBgHeight(e){
    	   var rollFlag= $("#rollFlag").val();
           var min = e.min;
           var max = e.max;
           var des = "";
           if(rollFlag == "1"){
        	   des =  accipiter.getLang_(messageLang,"set.bgHeight");
           }else{
        	   des =  accipiter.getLang_(messageLang,"set.bgWidth");
           }
           var html='<li><label>'+des+':</label><div class="set-chose-content set-bg"><input type="text" class="chose-btn up" readonly><ul>';
        ...
        * */
        ...
        function CreatePicture(){
        	...
        	var flag = $("#flag").val();
        	var rollFlag = $("#rollFlag").val();
        	if(textColor == "255,255,255,1" || background == "255,255,255,1"){
        		$(".imgContent").css({"background-color":"#eee"});
        	}else{
        		$(".imgContent").css({"background-color":"#fff"});
        	}
         ...
         //var text = encodeURI(encodeURI(text11));
        	...
        	if(isPurity==0 || isPurity==2){
                data = {"font":textStyle,"textSize":textSize,"textColor":textColor,"background":background,"imageHeight":imageHeight,"isPurity":isPurity,"text":text,"flag":flag,"rollFlag":rollFlag,"isBold":fontBold,"isItalic":fontItalic};
        	}else{
                data={"font":textStyle,"textSize":textSize,"textColor":textColor,"isPurity":isPurity,"text":text,"backImagePath":backImagePath,"flag":flag,"rollFlag":rollFlag,"isBold":fontBold,"isItalic":fontItalic};
        		if(backImagePath==""){
        			$(".imgContent").html("");
        	...
        	$.ajax({
        		...
        		success:function(data){
        			...
        			if(control=="0"){
                        ...
                        $("#font").attr("value",fontStyle);
                        $("#isBold").attr("value",fontBold);
                        $("#isItalic").attr("value",fontItalic);
            			if(isPurity==0 || isPurity==2){
            		 	...
            		 	var rollFlag = $("#rollFlag").val();
		            	if(rollFlag == "1" ){
		            		$(".imgContent").find("img").parent().css({"height":"90px","overflow-x":"auto"});
		            	}else{
		            		$(".imgContent").find("img").parent().css({"height":"200px","overflow-y":"auto"});
		            	}
        		        ...
        		        init: function() { 
        		        	var imgDropzone=this;
        		     		this.on("thumbnail",function(file){
    							...
    							var widthMax=size.sizeData[0].widthMax;
    							var widthMin=size.sizeData[0].widthMin; 
    							var fileSize = size.sizeData[0].fileSize;
        		            	if(file.width<widthMin||file.width>widthMax||file.height<highMin||file.height>highMax){
        		            	...
        		            	}else if(file.size > fileSize*1000000){
    		                		$(".uploadPicture").find(".uploadErrInfo").text(accipiter.getLang_(messageLang,"file.too.big"));
    		                		imgDropzone.removeFile(file);
    		                		$(".imgContent").html('');	
        		            ...
        		            this.on("success", function(file,data) {
        		            	...
        		            	var control=data.split("`")[0];
        		            	var rollFlag = $("#rollFlag").val();
        		            	if(choseItemType == "1" && rollFlag == "1" ){
        		            		$(".uploadPicture").find("img").parent().css({"height":"auto","overflow-x":"auto"});
        		            	}else{
        		            		$(".uploadPicture").find("img").parent().css({"height":"200px","overflow-y":"auto"});
        		            	}
        		            	if(control=="0"){
        ...
        * */
        ...
        function getRollPicture(){
        	...
        	if(id!=""){
                ...
                fontStyle=$("#font").val();//字体
                fontBold=$("#isBold").val();//是否加粗
                fontItalic=$("#isItalic").val();//是否倾斜
                heightSize = $("#heightSize").val();
                if(fontColor == "#ffffff" || rollBackground == "#4f81bd"){
            		$(".imgContent").css({"background-color":"#eee"});
            	}else{
            		$(".imgContent").css({"background-color":"#fff"});
            	}
        ...
        * */
        function clickSwitch(node){
            ...
            if(node.parent().hasClass("set-bg")){
                ...
                var rollFlag = $("#rollFlag").val();
                var  isPurity =$("#isPurity").val();
            	if(rollFlag == "2" && isPurity == "1"){
            		$(".uploadPicture").find("img").parent().css({"height":"200px","overflow-y":"auto"});
            	}else{
        			$(".uploadPicture").find("img").parent().css({"height":"auto","overflow-x":"auto"});    			
            	}
          ...
          * */
          function changeStyle(e){
              if(e.hasClass("up")){
				   ...
				   */
                  $(e).parent().find("ul").css("display","block");
              }else{
                  e.removeClass("down");
                  e.addClass("up");
				  /*
          ...
          $(".set-chose").on("click","input,li,td",function(){
              ...
              if($(this).hasClass("chose-item")){
				   ...
				   */
                  var e = $(this).parent().parent().find(".chose-btn");
                  e.attr("value",chose);
                  changeStyle(e);
                  ...
                  if($(this).parent().parent().hasClass("set-font-bold")){
                      fontBold=$(this).attr("value");
                      return;
                  }
                  if($(this).parent().parent().hasClass("set-font-italic")){
                      fontItalic=$(this).attr("value");
                      return;
                  }
