...
<head>
	...
	.createContent .title-table .set-chose li .set-chose-content ul li{width: 100%;height: 20px;line-height: 20px;padding-left: 0;text-indent: 7px;overflow: hidden;}
	...
	.createContent .title-table .set-chose .set-font-bold a{width:100px;height: 20px;text-indent: 7px;background-color: #f3f3f3;border: 1px solid #ccc;}
	.createContent .title-table .set-chose .set-font-bold input[type="file"]{opacity: 0;}
	.createContent .title-table .set-chose .set-font-italic a{width:100px;height: 20px;text-indent: 7px;background-color: #f3f3f3;border: 1px solid #ccc;}
	.createContent .title-table .set-chose .set-font-italic input[type="file"]{opacity: 0;}
    .createContent .title-table .set-chose .uploadPicture{width:100%;height: auto;padding-left:0}
    .createContent .title-table .set-chose .uploadPicture div{overflow: hidden;}
    ...
    .createContent .set-text .imgContent{width: 473px;height:auto;min-height: 60px;margin-bottom:10px;border-top: 1px solid #cccccc;background-color: #fff;overflow: hidden;overflow-x:auto}
	...
	.createContent .title-table .set-chose li .set-font-bold input{width:100px;}
	.createContent .title-table .set-chose li .set-font-bold ul{width:100px;}
	.createContent .title-table .set-chose li .set-font-bold{width:100px;position: relative;}
	.createContent .title-table .set-chose li .set-font-italic input{width:100px;}
	.createContent .title-table .set-chose li .set-font-italic ul{width:100px;}
	.createContent .title-table .set-chose li .set-font-italic{width:100px;position: relative;}
...
<body>
    ...
    <li>
		...
		<c:choose><c:when test="${not empty adType.id}"><spring:message code='control.update' /></c:when>
        			<c:otherwise><spring:message code='control.add' /></c:otherwise></c:choose></a></shiro:hasPermission>
     </li>
     </ul>
    </div>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/adv/control"><spring:message code='control.list' /></a></li>
		<shiro:hasPermission name="adv:material:edit"><li  class="active"><a href="${ctx}/adv/control/form?id=${adControll.id}">
		<c:choose><c:when test="${not empty adControll.id}"><spring:message code='control.update' /></c:when>
        			<c:otherwise><spring:message code='control.add' /></c:otherwise></c:choose></a></li></shiro:hasPermission>
	</ul>
	<form:form id="inputForm" modelAttribute="adControll" action="${ctx}/adv/control/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		<form:hidden path="rollText"/>
		<form:hidden path="rollTextSize"/>
		<form:hidden path="rollTextColor"/>
		<form:hidden path="font"/>
		<form:hidden path="isBold"/>
		<form:hidden path="isItalic"/>
		<form:hidden path="isPurity"/>
		<form:hidden path="rollBackground"/>
		<input id="heightSize" type="hidden" value="${adControll.height}">
		<tags:message content="${message}"/>
	 	 <c:if test="${isNotAdv}">
	 	 	<div class="control-group">
	 	 	<label class="control-label"><spring:message code='adv.user' />：</label>
	 	 	<div class="controls">
					<form:select path="advertiser.id" class="required">
		...
		<div class="control-group">
			<label class="control-label"><spring:message code='control.version' />:</label>
			<div class="controls">
				<form:input path="version" id="version" htmlEscape="false" maxlength="50" class="required"/>
			</div>
		</div>
	 	<div class="control-group">
		...
		<div class="control-group adType">
			<label class="control-label"><spring:message code='adv.type' />:</label>
			<div class="controls">
				<form:select path="adType.id" id="adType" class="select required">
					<option value=""> <spring:message code='userform.select' /></option>
		...
		<div class="control-group rollType" style="display:none">
			...
			<div class="controls">
			   <form:select path="rollFlag" class="select required">
		...
		<div class="control-group pictureType">
			...
			<form:hidden path="previewVedioPath"/> 	 
		...
		<c:if test="${not empty adControll.id}">
			<div class="control-group">
				<label class="control-label"><spring:message code='control.format' />:</label>
				<div class="controls">
					<label class="lbl" id="pictureType">${adControll.fileFormat}</label>
				</div>
			</div>
			<div class="control-group">
				...
				<div class="controls">
					<div class="createContent">
					     <div class="title-table">
					         <ul class="set-ul">
								 <li class="set-font-style"><a></a></li>
		...
		<div class="form-actions">
			<shiro:hasPermission name="adv:material:edit"><input id="btnSubmit" class="btn btn-primary" type="submit" value="<spring:message code='save' />"/>&nbsp;</shiro:hasPermission>
