...
<head>
	...
	<script adelement="text/javascript">
		...
		function page(n,s){
			$("#pageNo").val(n);
			$("#pageSize").val(s);
			$("#searchForm").attr("action","${ctx}/adv/adelement/auditStatictis/query");
			$("#searchForm").submit();
	    	return false;
	    }
	</script>
...
<body>
	...
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/adv/adelement"><spring:message code='audit.list' /></a></li>
		 
	</ul>
	<form:form id="searchForm" modelAttribute="adelement" action="${ctx}/adv/adelement/auditStatictis/query" method="post" class="breadcrumb form-search">
		...
		<div>
			...
			<div>
			<div class="query-item">
				<label><spring:message code='login.name' />：</label><form:input path="auditUser.loginName" htmlEscape="false" maxlength="30" class="input-small"/>
			</div>
			<div class="query-item">
				<label><spring:message code='user.name' />：</label><form:input path="auditUser.name" htmlEscape="false" maxlength="30" class="input-small"/> 		
			</div>
			<div class="query-item">
			...
			<div class="query-item">
				<label><spring:message code='adv.resolution' />：
	...
	<div class="tab_content aduitStatic">
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<th class="td-fore1"><spring:message code='adv.playstart' /></th>
		<th class="td-fore1"><spring:message code='adv.playend' /></th>
		<th class="td-fore1"><spring:message code='claim.time' /></th>
		<th class="td-control"><spring:message code='audit.status' /><i class="td-open"></i></th>
		<th class="td-fore2"><spring:message code='adv.audit.user.login' /></th>
		<th class="td-fore2"><spring:message code='adv.audit.user.name' /></th>
		<th class="td-fore2"><spring:message code='audit.date' /></th>
		...
		<c:forEach items="${page.list}" var="adelement">
			...
			<td>${adelement.adId}</td>
				...
				<c:choose>
					...
					<c:otherwise><td></td><td class="td-fore1"></td></c:otherwise>
				</c:choose>
				<c:choose>
				...
				<td class="td-fore1"><fmt:formatDate value="${adelement.endDate}" pattern="yyyy-MM-dd"/></td>
				<td class="td-fore1"><fmt:formatDate value="${adelement.claimDate}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
				<td>
					<c:choose>
						<c:when test="${adelement.status == -1}">
				...
				<c:choose>
						<c:when test="${!empty adelement.auditUser}">
							<td class="td-fore2">${adelement.auditUser.loginName}</td>
						</c:when>
				...
				<td class="td-fore2"><fmt:formatDate value="${adelement.auditDate}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
				<td class="td-fore2">${adelement.remarks}</td>
				<shiro:hasPermission name="adv:aduit:statistic:view">
					<td>
						 
						 
						<a href="${ctx}/adv/adelement/auditStatictis/form?id=${adelement.id}"><spring:message code='detail' /></a>
