...
<script type="text/javascript">
	$("#${id}Button").click(function(){
		...
		// 正常打开	
		top.$.jBox.open("iframe:${ctx}/tag/treeselectnotFirst?url="+encodeURIComponent("${url}")+"&module=${module}&checked=${checked}&extId=${extId}&nodesLevel=${nodesLevel}&selectIds="+$("#${id}Id").val(), "<spring:message code='treeselect.selection' />"+" "+"<spring:message code='${title}' />", 300, 420, {
			buttons:{"<spring:message code='treeselect.confirm' />":"ok", ${allowClear?"\"<spring:message code='treeselect.clear' />\":\"clear\", ":""}"<spring:message code='treeselect.colse' />":true}, submit:function(v, h, f){
				if (v=="ok"){
					...
					var span = h.prevObject[0].lastChild.firstChild;
					var ids = [], names = [], nodes = [];
					var myp = document.createElement("p");
					var extp =span.firstChild;
					if(extp){
						span.removeChild(extp);
					}
					if ("${checked}" == "true"){
					...
					if("${id}"=="adProgramCategoryLevel1" && nodes[0].level != 1){
						myp.style.color = 'red';
						myp.style.fontSize='15px';
						myp.innerHTML = "Please select first-grade category";
						span.appendChild(myp);
						return false;
					}
					if("${id}"=="adProgramCategoryLevel2" && nodes[0].level == 1){
						myp.innerHTML = "Please select second-grade category";
						myp.style.color = 'red';
						myp.style.fontSize='15px';
						span.append(myp);
						return false;
