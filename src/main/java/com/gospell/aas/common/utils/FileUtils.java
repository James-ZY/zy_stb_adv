...
public class FileUtils extends org.apache.commons.io.FileUtils {
	...
	public static String getAbsolutePath(String urlPath) {
		if (StringUtils.isBlank(urlPath)) {
			...
			return ApplicationContextHelper.getRootRealPath()
	...
	public static String getFullFileUrl(String url){
		return "http://"+GlobalUtil.getIpAddress()+":8080" +url;
	}
	...
	public static String getFileUrl2(String url) {
	...
	public static String getFileRealPath(String url) {
 
		 return ApplicationContextHelper.getRootRealPath()+getFileUrl2(url);
	}
	
	/**
	 * 获取项目部署的路径（目前开发部署的是：D:\apache-tomcat-7.0.62\webapps）
	 * 
	 * @param url 数据库存取的路径
	 * @return
	 */
	...
	public static String getUploadFileRealPath() {
		String root = ApplicationContextHelper.getRootRealPath();
		if(root != null && root.contains("\\")){
			root = root.replace("\\", "/");
		}
		String[] roots = root.split("/");
		String[] uploadFilePaths = new String[roots.length-1];
		for (int i = 0; i < roots.length-1; i++) {
			uploadFilePaths[i] = roots[i];
		}
		String uploadFilePath = StringUtils.join(uploadFilePaths, "/");
	...
	public static byte[] getFileByte(String filename) {
		...
		if (StringUtils.isBlank(filename)){
			pathname = ApplicationContextHelper.getRootRealPath()+"/static/images/gospell.png";
			 
		}	
		else
			pathname = ApplicationContextHelper.getRootRealPath()+getFileUrl2(filename);
	 ...
	 /**
     * 分离路径，必须上传的文件路径放在tomcat部署项目的相同路径，否则每回重新部署，以前上传的数据就会丢失
     * 现在advs的部署路径是D:\apache-tomcat-7.0.62\webapps，那么在下面应该还有一个upload来保存advs的上传数据
     * @return 
     */
    public static String[] fileUploadAdr(String rootpath){
    	log.info("分离的rootpath:"+rootpath);
    	//System.out.println("分离的rootpath:"+rootpath);
    	if(rootpath != null && rootpath.contains("\\")){
    		rootpath = rootpath.replace("\\", "/");
    	}
    
    	String[] s=rootpath.split("/");
    	 
    	String[] real = new String[s.length-1];
    	...
    	//保证了每个用户上传不会相互影响
    	String realPath = path+"/upload/"+s[s.length-1];
	 ...
	 * @return
	 */
	public static BigDecimal getFileSize(String fileName){
		File file = new File(fileName);
		if (file.isFile()) {
			long length = file.length();
			long kb = length/1024;
			long kb1 = (length%1024)/100;
			long l = (length%1024)%100;
			long kb2 = l/10;
			long kb3 =  l%10;
			String s = kb +"."+kb1+kb2+kb3;
	...
	public static void main(String[] args) {
		/*String path ="D:\\apache-tomcat-7.0.62\\webapps\\upload\\advs\\admin\\m2v\\admin_20160825143501627.m2v";
	 
		BigDecimal file_con = com.gospell.aas.common.utils.FileUtils.getFileSize(path);
		BigDecimal max = new BigDecimal(64);
		int compare = file_con.compareTo(max);
		if(compare == 1){
			System.out.println("dayu");
		}*/
		getFileByte(null);
/*		copyFileCover("D:\\apache-tomcat-7.0.62\\webapps\\advs\\static\\images\\bg1.jpg", "D:\\apache-tomcat-7.0.62\\webapps\\advs\\static\\images\\bg2.jpg", true);
*/	}
