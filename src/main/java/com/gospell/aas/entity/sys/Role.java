...
public class Role extends IdEntity<Role> {
    ...
    private List<Menu> menuList = Lists.newArrayList(); // 拥有菜单列表
     ...
     private List<Advertiser> advertiserList = Lists.newArrayList();//拥有的广告商列表
    ...
    public static final String AUDIT_ID="1";
	...
	@ManyToMany(fetch = FetchType.EAGER)
    @JoinTable(name = "ad_advertiser_role", joinColumns = { @JoinColumn(name = "ad_role_id") }, inverseJoinColumns = { @JoinColumn(name = "ad_advertiser_id") })
    @Where(clause = "del_flag='" + DEL_FLAG_NORMAL + "'")
    @OrderBy("id")
    @Fetch(FetchMode.SUBSELECT)
    @NotFound(action = NotFoundAction.IGNORE)
